"""
Flash Attention implementation in Triton for Ampere and newer GPUs.

This implementation uses pointer-based memory access which works on all
CUDA GPUs (Ampere, Hopper, Blackwell, etc.).

Based on the official Triton Flash Attention tutorial:
...
def _attn_fwd_inner(
    ...
    # Advance block pointers to the starting position
...
NUM_STAGES_OPTIONS = [1] if is_hip() else [2, 3]
...
@pytest.mark.parametrize("N_CTX", [128, 1024])
@pytest.mark.parametrize("HEAD_DIM", [64, 128])
@pytest.mark.parametrize("causal", [False, True])
@pytest.mark.parametrize("mode", ["fwd", "bwd"])
def test_op(Z, H, N_CTX, HEAD_DIM, causal, mode, dtype=torch.float16):
