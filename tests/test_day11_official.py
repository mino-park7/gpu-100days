    ...
    def forward(ctx, q, k, v, causal, sm_scale, warp_specialize=True):
        ...
        if supports_host_descriptor() and not (is_hopper() and warp_specialize):
            ...
            desc_q = TensorDescriptor(
                q, shape=[y_dim, HEAD_DIM_K], strides=[HEAD_DIM_K, 1], block_shape=dummy_block
            )
            ...
            desc_k = TensorDescriptor(
                k, shape=[y_dim, HEAD_DIM_K], strides=[HEAD_DIM_K, 1], block_shape=dummy_block
            )
            desc_o = TensorDescriptor(
                o, shape=[y_dim, HEAD_DIM_K], strides=[HEAD_DIM_K, 1], block_shape=dummy_block
            )
        else:
            desc_q = q
            desc_v = v
